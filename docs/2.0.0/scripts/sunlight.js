...
(function (window, document, undefined) {
    ...
    return {
      ...
      handle_ident: function (context) {
        ...
        return (
          ...
          evaluate(
            context.language.namedIdentRules.follows,
            function (ruleData) {
          ...
          evaluate(
            context.language.namedIdentRules.precedes,
            function (ruleData) {
              return createProceduralRule(
                context.index + 1,
                1,
          ...
          evaluate(
            context.language.namedIdentRules.between,
            function (ruleData) {
              return createBetweenRule(
                context.index,
                ruleData.opener,
                ruleData.closer,
                context.language.caseInsensitive,
    ...
    return function (tokens) {
      ...
      while ((token = tokens[--index]) !== undefined) {
        if (
          ...
          contains(closer.values, token.value)
        ) {
          if (
      ...
      if (word === peek || wordMap[i].regex.test(peek)) {
        return context.createToken(
          ...
          context.reader.current() +
            context.reader[doNotRead ? "peek" : "read"](word.length - 1),
          line,
          column,
        );
      }
    ...
    function switchToEmbeddedLanguageIfNecessary(context) {
      ...
      for (i = 0; i < context.language.embeddedLanguages.length; i++) {
        ...
        if (embeddedLanguage.switchTo(context)) {
          ...
          context.items = merge(
            context.items,
            clone(context.language.contextItems),
          );
          break;
        }
      }
    ...
    return {
      ...
      highlightNode: function highlightRecursive(node) {
        ...
        if (
          ...
          (match = this.matchSunlightNode(node)) === null
        ) {
          return;
        }
  ...
  window.Sunlight.registerLanguage(DEFAULT_LANGUAGE, {
    punctuation: /(?!x)x/,
    numberParser: EMPTY,
  });
})(this, document);
