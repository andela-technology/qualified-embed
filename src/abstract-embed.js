...
export class AbstractEmbed {
	...
	destroy() {
		...
		this.iframe = this.node = this.manager = this.options = this.connection = this.childMethods = null;
	}
	...
	_post(method, data) {
		...
		if(!this.childMethods || !this.childMethods[method]) {
			return Promise.reject(Error(`Unable to call method "${method}" on iframe`));
		}
