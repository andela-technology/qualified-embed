...
export class QualifiedEmbeddedChallenge extends AbstractEmbed {
	...
	constructor({ node, challengeId, options = {}, manager }) {
		...
		if(!challengeId) {
			throw new Error('Unable to determine challenge ID from node: no value for data-qualified-embed');
		}
		...
		this._initPenpal(['ready', 'loaded', 'change', 'runStart', 'run'], ['challengeId', 'manager']);
	}
	 ...
	 */
	update({ challengeId = this.challengeId, options = null, reload = false } = {}) {
	...
	onReady() {
		if(this.options.localStorageId) {
			const savedData = _getStorageData(this.options.localStorageId, this.challengeId);
			if(savedData && savedData.files) {
				this.options.initialFiles = Object.assign({}, this.options.initialFiles, savedData.files);
				if(savedData.cursor) {
	...
	_updateIframe() {
		...
		this.iframe.src = concatUrl(baseURL, 'embed', this.challengeId) + '?' + toQueryParams(params);
	}
}
