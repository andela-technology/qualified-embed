...
export class QualifiedEmbeddedAssessment extends AbstractEmbed {
	...
	constructor({ node, options }) {
		...
		if(!options || !options.invitePath || !options.authToken) {
			throw new Error('Unable to load an assessment without invitePath and authToken');
		}
		...
		this._initPenpal(['ready', 'error', 'loaded', 'updated', 'solutionUpdated', 'submitted']);
	}
	...
	update({ options = null, reload = false } = {}) {
		...
		let changed = false;
		if(options) {
			changed = (options.invitePath && options.invitePath !== this.options.invitePath);
	...
	_updateIframe() {
		...
		const embedPath = this.options.invitePath.replace(/^(\/assess\/)([a-fA-F0-9]{24})/, (_, path, assessmentId) => {
